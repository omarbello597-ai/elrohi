...
function printFormato(fc, logoBase64='') {
  ...
  const garmentRows = (lot.garments||[]).map((g,i)=>{
    ...
    return `<tr><td style="border:1px solid #1a3a6b;padding:4px 8px;font-size:10px;font-weight:600;color:#1a3a6b;min-width:160px">${g.descripcionRef||gLabel(g.gtId)}</td>${sizes}<td style="border:1px solid #1a3a6b;padding:3px 4px;text-align:center;font-size:10px;font-weight:700;background:#dce6f5">${g.total?.toLocaleString('es-CO')||''}</td><td style="border:1px solid #1a3a6b;padding:4px 8px;font-size:9px;color:#374151">${g.comentario||''}</td></tr>`;
  }).join('');
  const emptyRef = Array(Math.max(0,5-(lot.garments||[]).length)).fill(0).map(()=>`<tr><td style="border:1px solid #1a3a6b;height:22px;min-width:160px"></td>${activeSizes.map(s=>'<td style="border:1px solid #1a3a6b"></td>').join('')}<td style="border:1px solid #1a3a6b;background:#dce6f5"></td><td style="border:1px solid #1a3a6b;background:#f5f0fa"></td></tr>`).join('');
  ...
  <div style="max-width:960px;margin:10px auto;border:1.5px solid #1a3a6b">
    ...
    <table style="width:100%;border-collapse:collapse;"><thead><tr><th style="width:160px;border:1px solid #1a3a6b;padding:4px 8px;background:#deeaf5;font-size:9px;font-weight:700;color:#14405A;text-align:left">Referencia</th>${activeSizes.map(s=>`<th style="border:1px solid #1a3a6b;padding:3px 4px;background:#e8eef7;font-size:9px;font-weight:700;color:#1a3a6b;text-align:center;white-space:nowrap">${s}</th>`).join('')}<th style="width:46px;border:1px solid #1a3a6b;padding:3px 2px;background:#c5daf0;font-size:9px;font-weight:700;color:#14405A;text-align:center">TOTAL</th><th style="width:180px;border:1px solid #1a3a6b;padding:3px 6px;background:#f5f0fa;font-size:9px;font-weight:700;color:#4a3a6b;text-align:center">Comentarios / Novedades</th></tr></thead><tbody>${garmentRows}${emptyRef}</tbody></table>
