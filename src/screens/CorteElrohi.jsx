...
function printFormato(fc, logoBase64='') {
  ...
  const garmentRows = (lot.garments||[]).map((g,i)=>{
    const sizes = activeSizes.map(s=>{const val=g.sizes?.[s]||''; return `<td style="border:1px solid #1a3a6b;padding:3px 5px;text-align:center;font-size:10px;font-weight:600;color:#1a3a6b;white-space:nowrap;width:1%">${val||''}</td>`;}).join('');
    return `<tr><td style="border:1px solid #1a3a6b;padding:4px 8px;font-size:10px;font-weight:600;color:#1a3a6b;white-space:nowrap">${g.descripcionRef||gLabel(g.gtId)}</td>${sizes}<td style="border:1px solid #1a3a6b;padding:3px 4px;text-align:center;font-size:10px;font-weight:700;background:#dce6f5">${g.total?.toLocaleString('es-CO')||''}</td><td style="border:1px solid #1a3a6b;padding:4px 8px;font-size:9px;color:#374151">${g.comentario||''}</td></tr>`;
  }).join('');
  const emptyRef = Array(Math.max(0,5-(lot.garments||[]).length)).fill(0).map(()=>`<tr><td style="border:1px solid #1a3a6b;height:22px;white-space:nowrap"></td>${activeSizes.map(s=>'<td style="border:1px solid #1a3a6b"></td>').join('')}<td style="border:1px solid #1a3a6b;background:#dce6f5"></td><td style="border:1px solid #1a3a6b;background:#f5f0fa"></td></tr>`).join('');
  ...
  <div style="max-width:960px;margin:10px auto;border:1.5px solid #1a3a6b">
    <div style="border-bottom:2px solid #14405A;padding:8px 14px;display:flex;align-items:center;justify-content:space-between;gap:12px;background:#F7F7F7">
      <img src="${logoBase64}" style="width:110px;height:80px;object-fit:contain;border-radius:4px" />
    ...
    <table style="border-collapse:collapse;table-layout:auto;"><thead><tr><th style="border:1px solid #1a3a6b;padding:4px 8px;background:#deeaf5;font-size:9px;font-weight:700;color:#14405A;text-align:left">Referencia</th>${activeSizes.map(s=>`<th style="border:1px solid #1a3a6b;padding:3px 5px;background:#e8eef7;font-size:9px;font-weight:700;color:#1a3a6b;text-align:center;white-space:nowrap;width:1%">${s}</th>`).join('')}<th style="border:1px solid #1a3a6b;padding:3px 8px;background:#c5daf0;font-size:9px;font-weight:700;color:#14405A;text-align:center;white-space:nowrap">TOTAL</th><th style="border:1px solid #1a3a6b;padding:3px 8px;background:#f5f0fa;font-size:9px;font-weight:700;color:#4a3a6b;text-align:center;width:99%">Comentarios / Novedades</th></tr></thead><tbody>${garmentRows}${emptyRef}</tbody></table>
